...
namespace NSPartUtils
{
	...
	/**
	 * 스탯 수치 표시 공용 포맷: 스탯 표시 DT의 DisplayUnit(%, 초)을 붙이고, bShowDirection이면 수치 부호로
	 * "증가"/"감소" 라벨을 덧붙임(이때 수치는 절댓값으로 표시해 부호와 라벨이 겹치지 않게 함).
	 * 범위(Min~Max) 프리뷰처럼 방향이 아직 확정되지 않은 표시는 bShowDirection=false로 단위만 적용
	 */
	NEOSANCTUM_API FText FormatStatValueText(
		const UObject* WorldContextObject, const FGameplayTag& StatTag, float Value, bool bShowDirection = true);
}
