...
public:
	...
	// ABP가 바라볼 고정 조준 위치를 서버에서 클라이언트로 복제하는 함수
	void SetReplicatedAimTargetLocation(const FVector& InLocation);

	// ABP 고정 조준 위치를 해제하는 함수
	void ClearReplicatedAimTargetLocation();

	// ABP가 사용할 고정 조준 위치를 가져오는 함수
	bool TryGetReplicatedAimTargetLocation(FVector& OutLocation) const;
	...
	bool HasCurrentAttackRow() const { return bHasCurrentAttackRow || !ReplicatedAttackId.IsNone(); }
...
private:
	...
	// 클라이언트 ABP가 고정 조준 위치를 사용해야 하는지 나타내는 변수
	UPROPERTY(Replicated)
	bool bHasReplicatedAimTargetLocation = false;

	// 클라이언트 ABP가 바라볼 고정 조준 위치 변수
	UPROPERTY(Replicated)
	FVector_NetQuantize ReplicatedAimTargetLocation = FVector::ZeroVector;
};
