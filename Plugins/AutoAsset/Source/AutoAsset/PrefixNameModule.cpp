	...
	if (EditorUtilitySubsystem)
	{
		...
		for (const FString& PathStr : StartupBPPaths)
		{
			...
			if (Asset)
			{
				...
				{
					UE_LOG(LogTemp, Warning, TEXT("[AutoAsset] Failed to execute Editor Utility (TryRun failed): %s"),
					       *PathStr);
				}
			}
			else
			{
				UE_LOG(LogTemp, Error,
				       TEXT("[AutoAsset] Failed to load Editor Utility asset: %s. Is the plugin content available?"),
				       *PathStr);
			}
		}
	}
